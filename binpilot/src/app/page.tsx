...
export default function BinPilotPage() {
  ...
  const pathway = [
    { icon: Send,          step: '1', title: 'Book in 30 Seconds',        desc: 'Pick your bins, choose monthly or one-time, drop your address. Done. We pre-fill your clean date.',            color: 'text-teal-600',   bg: 'bg-teal-500/10 border-teal-500/20' },
    { icon: MessageSquare, step: '2', title: 'Steve Texts You Back',       desc: 'A real local neighbour confirms your spot — not a call centre, not a chatbot.',                               color: 'text-sky-600',    bg: 'bg-sky-500/10 border-sky-500/20' },
    { icon: Calendar,      step: '3', title: 'Wednesday: We Come to You', desc: 'Bins emptied Tuesday. We arrive Wednesday and hit them with a 200°F pressure wash right at your curb.',       color: 'text-orange-600', bg: 'bg-orange-500/10 border-orange-500/20' },
    { icon: Camera,        step: '4', title: 'Before & After Photo',       desc: 'Proof on your phone. Bins back in place. You didn\'t lift a finger or a hose.',                               color: 'text-violet-600', bg: 'bg-violet-500/10 border-violet-500/20' },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#faf7f0] text-slate-900">
      ...
      {/* ── Nav ── */}
      <header className="sticky top-0 left-0 right-0 z-50 bg-[#faf7f0]/90 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14">
          <a href="#" className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-teal-500 flex items-center justify-center">
              <Trash2 size={14} className="text-slate-900" />
            </div>
            <span className="font-bold tracking-tight">Bin<span className="text-teal-600">Pilot</span></span>
          </a>

          <nav className="hidden md:flex items-center gap-6 text-sm text-slate-600">
            {navLinks.map(l => <a key={l.label} href={l.href} className="hover:text-slate-900 transition-colors">{l.label}</a>)}
          </nav>
        ...
        {mobileOpen && (
          <div className="md:hidden border-t border-slate-200 bg-[#faf7f0] px-4 py-4 space-y-3">
            {navLinks.map(l => (
              <a key={l.label} href={l.href} className="block text-slate-700 hover:text-slate-900 py-1 text-sm" onClick={() => setMobileOpen(false)}>{l.label}</a>
            ))}
            <a href="#book" className="block mt-2 bg-gradient-to-r from-orange-500 to-amber-500 text-slate-900 font-bold text-sm px-4 py-2.5 rounded-lg text-center" onClick={() => setMobileOpen(false)}>
      ...
      <section className="relative flex items-center justify-center overflow-hidden pt-16 pb-12 sm:pt-20">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center fade-in-up">
          <div className="inline-flex items-center gap-2 bg-orange-500/10 border border-orange-500/25 text-orange-600 text-xs font-semibold px-3 py-1.5 rounded-full mb-7 tracking-wide">
            <span className="live-dot w-1.5 h-1.5 rounded-full bg-orange-400 inline-block" />
          ...
          <p className="text-lg sm:text-xl text-slate-600 max-w-xl mx-auto mb-9 leading-relaxed">
            A 200°F pressure wash at your curb — bacteria, smell and flies gone.
            <strong className="text-slate-900"> You never touch it.</strong>
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mb-7">
            ...
            <a href="sms:+15197293673&body=BIN CLEAN" className="inline-flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-100 border border-slate-300 text-slate-900 font-bold px-8 py-4 rounded-xl text-lg transition-colors">
              <Phone size={18} /> Text BIN CLEAN
            </a>
          </div>

          {serviceDate && (
            <div className="inline-flex items-center gap-2 text-sm font-medium text-teal-600 bg-teal-500/10 border border-teal-500/20 px-4 py-2 rounded-full">
              <Calendar size={14} />
          ...
          <div className="mt-9 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-sm text-slate-500">
            <div className="flex items-center gap-1.5"><ShieldCheck size={14} className="text-teal-600" /><span>200°F sanitizing wash</span></div>
            <div className="flex items-center gap-1.5"><CheckCircle size={14} className="text-teal-600" /><span>Zero effort on your part</span></div>
            <div className="flex items-center gap-1.5"><Heart size={14} className="text-teal-600" /><span>Local family — Steve lives here</span></div>
          </div>
          ...
          <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-3xl mx-auto">
            {[
              ...
              <figure key={p.src} className="relative rounded-2xl overflow-hidden border border-slate-200 bg-white">
                <img src={p.src} alt="A bin before and after a 200°F sanitizing clean" className="w-full h-full object-cover" />
                <span className="absolute top-2.5 left-2.5 bg-[#0b0f17]/80 backdrop-blur-sm border border-slate-300 text-white text-[9px] font-black tracking-widest px-2 py-1 rounded">BEFORE → AFTER</span>
                <span className="absolute bottom-2.5 right-2.5 bg-[#0b0f17]/70 text-slate-100 text-[10px] font-bold px-2 py-0.5 rounded">{p.cap}</span>
              </figure>
            ))}
          </div>
        </div>
      ...
      <div className="border-y border-slate-200 bg-[#fbf9f3]">
        <BinConfessional />
      </div>

      {/* ── Raccoon Inspection Failed ── */}
      <section className="py-6 max-w-5xl mx-auto px-4 sm:px-6 mt-8">
        <div className="bg-orange-50 border-2 border-orange-300 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row gap-6 items-start">
          <div className="flex-shrink-0">
            <img src="/raccoons/inspector.png" alt="Cartoon raccoon inspector in a hi-vis vest" loading="lazy"
              className="w-24 h-24 sm:w-28 sm:h-28 object-cover rounded-xl bg-white border border-orange-500/30" />
          </div>
          <div className="flex-1">
            ...
            <h3 className="font-black text-xl text-orange-700 mb-1">Property Address: <span className="text-slate-900">Yours</span></h3>
            <p className="text-orange-600 text-sm mb-3">Suspicious smell · excessive grime · fly traffic over the legal limit.</p>
            <div className="flex flex-wrap gap-3 text-sm">
              <span className="bg-teal-500/15 border border-teal-500/30 text-teal-600 px-3 py-1.5 rounded-lg font-semibold">✓ A cleaner neighbourhood</span>
              <span className="bg-teal-500/15 border border-teal-500/30 text-teal-600 px-3 py-1.5 rounded-lg font-semibold">✓ A happier you</span>
              <span className="bg-teal-500/15 border border-teal-500/30 text-teal-600 px-3 py-1.5 rounded-lg font-semibold">✓ Less raccoon activity</span>
            </div>
          </div>
          <div className="flex-shrink-0 text-center">
            <p className="text-xs text-orange-600/60 mb-2 font-semibold">RECOMMENDED TREATMENT</p>
      ...
      {/* ── The Problem ── */}
      <section id="problem" className="py-20 bg-white border-y border-slate-200 mt-8">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 bg-red-500/10 border border-red-500/20 text-red-600 text-xs font-semibold px-3 py-1.5 rounded-full mb-4 tracking-wide">
            ...
            <p className="text-slate-600 max-w-xl mx-auto">A few feet from your front door. Right next to where your family lives.</p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            {threats.map(t => (
              <div key={t.label} className="bg-white border border-red-500/10 rounded-xl p-6 hover:border-red-500/25 transition-colors">
                <div className="w-10 h-10 rounded-lg bg-red-500/10 flex items-center justify-center mb-4">
                  <t.icon size={18} className="text-red-600" />
                </div>
          ...
          <div className="bg-teal-500/10 border border-teal-500/20 rounded-2xl p-6 sm:p-8 text-center max-w-3xl mx-auto">
            <Sparkles size={22} className="text-teal-600 mx-auto mb-3" />
            <p className="text-lg sm:text-xl font-bold mb-1">Your bin doesn&apos;t have to be a biohazard.</p>
            <p className="text-slate-600 text-sm mb-5">One clean and the smell, the bugs, and the bacteria are gone. We keep it that way.</p>
      ...
      <section id="pricing" className="py-20 max-w-2xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-teal-500/10 border border-teal-500/20 text-teal-600 text-xs font-semibold px-3 py-1.5 rounded-full mb-4 tracking-wide">
            NEIGHBOURHOOD INTRO OFFER
          </div>
          <h2 className="text-3xl sm:text-4xl font-black mb-3">What you get</h2>
          <p className="text-slate-600">Every clean includes everything. No tiers, no upsells, no fine print.</p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-8 mb-6">
          <ul className="space-y-4">
            {[
              ...
              <li key={item.t} className="flex items-start gap-3 text-slate-700 text-sm">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            ...
            <div key={p.bins} className={`rounded-2xl p-5 text-center ${p.highlight ? 'bg-teal-500/10 border-2 border-teal-500/40' : 'bg-white border border-slate-200'}`}>
              <span className="text-slate-600 text-xs font-medium block mb-1">{p.bins}</span>
        ...
        <p className="text-center text-xs text-slate-500 mt-4">
          Multi-unit buildings — <a href="mailto:binpilotmimico@gmail.com" className="text-teal-600 hover:underline">email us for a quote</a>.
        </p>
      </section>
      ...
      <section className="py-20 bg-white border-y border-slate-200">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-600">Real results from real Mimico and New Toronto homes.</p>
          </div>
      ...
      <section id="book" className="py-20 max-w-2xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-600">We already picked your date. Just confirm the details.</p>
        </div>

        {fpState.succeeded ? (
          <div className="text-center py-14 bg-teal-500/10 border border-teal-500/20 rounded-2xl px-8">
            <span className="text-5xl block mb-5">🎉</span>
            <h3 className="text-2xl font-black mb-3 text-teal-600">You&apos;re in!</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="flex items-center gap-3 bg-teal-500/10 border border-teal-500/30 rounded-xl px-5 py-4">
              <Calendar size={20} className="text-teal-600 flex-shrink-0" />
              <div className="flex-1">
                <p className="text-xs text-slate-600">Your clean date — right after Tuesday pickup</p>
                <p className="font-bold text-teal-600 text-lg">{serviceDate || 'This Wednesday'}</p>
              </div>
              <div className="text-right hidden sm:block">
                <p className="text-xs text-slate-500">Spots remaining</p>
                <p className="text-orange-600 font-black text-xl count-up">{spotsLeft}</p>
              </div>
            </div>

            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">1. Which bins?</label>
              <div className="grid grid-cols-3 gap-3">
                {binOptions.map(bin => (
                  <button key={bin} type="button" onClick={() => toggleBin(bin)}
                    className={`py-3.5 rounded-xl text-sm font-bold border transition-all ${form.bins.includes(bin) ? 'bg-teal-500/20 border-teal-500/50 text-teal-600 scale-[1.02]' : 'bg-slate-100 border-slate-200 text-slate-600 hover:border-teal-200'}`}>
                    {bin}
                  </button>
                ))}
              </div>
              {binCount > 0 && (
                <p className="text-teal-600 font-black text-lg mt-2 count-up">${total} total{form.plan === 'Monthly' ? ' per clean' : ''}</p>
              )}
            </div>

            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">2. How often?</label>
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button key={opt.id} type="button" onClick={() => setForm(f => ({ ...f, plan: opt.id }))}
                    className={`px-4 py-3.5 rounded-xl text-left border transition-all ${form.plan === opt.id ? 'bg-teal-500/20 border-teal-500/50 scale-[1.01]' : 'bg-slate-100 border-slate-200 hover:border-slate-300'}`}>
                    <span className={`block text-sm font-bold ${form.plan === opt.id ? 'text-teal-600' : 'text-slate-700'}`}>{opt.label}</span>
            ...
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">3. Where?</label>
              <div className="space-y-3">
                <input required type="text" placeholder="Street address (Mimico or New Toronto)" value={form.address}
                  onChange={e => setForm(f => ({ ...f, address: e.target.value }))}
                  className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-600 focus:outline-none focus:border-teal-500/50 transition-colors" />
                ...
                {showNotes ? (
                  <div className="space-y-3">
                    ...
                    <textarea rows={2} placeholder="Gate code, where bins live, different preferred day..."
                      ...
                      className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-600 focus:outline-none focus:border-teal-500/50 transition-colors resize-none" />
                  </div>
                ) : (
                  <button type="button" onClick={() => setShowNotes(true)} className="text-xs text-slate-500 hover:text-slate-700 transition-colors">
            ...
            <button type="submit" disabled={fpState.submitting || binCount === 0}
              className="glow-pulse-orange w-full bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-400 hover:to-amber-400 disabled:opacity-50 disabled:cursor-not-allowed text-slate-900 font-black py-4 rounded-xl transition-all text-lg">
              {fpState.submitting ? 'Booking...' : binCount === 0 ? 'Tap your bins above to start' : `Book My Clean — $${total}`}
            </button>

            <p className="text-center text-xs text-slate-500">
              Rather text? Send <span className="text-slate-600 font-bold">BIN CLEAN</span> to{' '}
              <a href="sms:+15197293673&body=BIN CLEAN" className="text-teal-600 font-bold">519-729-3673</a>
            </p>
          </form>
        )}
      </section>
      ...
      <section id="how-it-works" className="py-20 bg-white border-y border-slate-200">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-14" data-reveal>
            ...
            <p className="text-slate-600 max-w-xl mx-auto">From booking to spotless bins — and you lift exactly nothing.</p>
          </div>
      ...
      <section className="py-16 max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-10" data-reveal>
          ...
          <p className="text-slate-600">Five steps. You do exactly none of them.</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4" data-reveal>
          {[
            ...
            <div key={s.label} className="text-center">
              <div className="rounded-2xl bg-white border border-slate-200 p-2 mb-3 aspect-square flex items-center justify-center overflow-hidden">
                <img src={s.img} alt={s.label} loading="lazy" className="w-full h-full object-contain" />
              </div>
              <span className="text-xs font-black text-teal-600 tracking-widest">STEP {i + 1}</span>
      ...
      <section className="py-20 max-w-5xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12" data-reveal>
          ...
          <p className="text-slate-600 max-w-xl mx-auto">Real bins, real homes. The same bin — before, and after our 200°F sanitizing clean.</p>
        </div>
        <div className="grid sm:grid-cols-2 gap-5">
          {[
            ...
            <figure key={p.src} data-reveal className="rounded-2xl overflow-hidden border border-slate-200 bg-white">
              <div className="relative">
                <img src={p.src} alt="A bin shown before and after a 200°F sanitizing clean" loading="lazy" className="w-full h-auto block" />
                <span className="absolute top-3 left-3 bg-[#0b0f17]/80 backdrop-blur-sm border border-slate-300 text-white text-[10px] font-black tracking-widest px-2.5 py-1 rounded">BEFORE → AFTER</span>
              </div>
              <figcaption className="px-5 py-3.5 text-sm text-slate-600">{p.cap}</figcaption>
            </figure>
          ))}
        </div>
        <p className="text-center text-xs text-slate-500 mt-6">Actual results from bins we&apos;ve cleaned. Your bin&apos;s glow-up is one tap away.</p>
      </section>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid md:grid-cols-3 gap-5 mb-10" data-reveal>
            {[
              ...
              <div key={f.title} className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-teal-500/20 transition-colors">
                <img src={f.img} alt="" loading="lazy" className="w-20 h-20 object-cover rounded-xl bg-white mb-4" />
      ...
      <section className="py-12 max-w-lg mx-auto px-4 sm:px-6 grid grid-cols-3 gap-4">
        {[
          { icon: Trash2,  label: 'Garbage',   color: 'text-slate-600' },
          { icon: Recycle, label: 'Recycling',  color: 'text-sky-600' },
          { icon: Leaf,    label: 'Green Bin',  color: 'text-teal-600' },
        ].map(b => (
          <div key={b.label} className="text-center bg-white border border-slate-200 rounded-xl py-5 px-3">
      ...
      <section id="faq" className="py-20 bg-white border-y border-slate-200">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-14" data-reveal>
            ...
            <p className="text-slate-600">Everything you need before you book.</p>
          </div>
          <div className="space-y-3">
            {faqs.map((f, i) => (
              <div key={i} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                <button className="w-full flex items-center justify-between px-6 py-5 text-left" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                  ...
                  <ChevronDown size={16} className={`text-slate-600 flex-shrink-0 transition-transform ${openFaq === i ? 'rotate-180' : ''}`} />
                </button>
                {openFaq === i && <div className="px-6 pb-5"><p className="text-slate-600 text-sm leading-relaxed">{f.a}</p></div>}
              </div>
            ))}
          </div>
        </div>
        ...
        <div data-reveal className="bg-gradient-to-br from-teal-500/10 to-transparent border border-teal-500/20 rounded-3xl p-6 sm:p-10 flex flex-col sm:flex-row items-center sm:items-start gap-7">
          <div className="flex-shrink-0 w-full sm:w-44 space-y-3">
            ...
            <img src="/photos/steve-builder.jpeg" alt="Steve on a local build site" loading="lazy"
              className="hidden sm:block w-44 h-28 object-cover rounded-xl border border-slate-200"
              onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />
          </div>
          <div className="text-center sm:text-left">
            <div className="inline-flex items-center gap-2 bg-teal-500/10 border border-teal-500/20 text-teal-600 text-xs font-semibold px-3 py-1.5 rounded-full mb-3 tracking-wide">
            ...
            <p className="mt-4 text-teal-600 font-bold text-sm">— Steve · Mimico, since 2021</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-28 max-w-4xl mx-auto px-4 sm:px-6 text-center">
        <div className="relative">
          ...
          <div className="relative">
            ...
            <p className="text-slate-600 text-lg mb-3 max-w-xl mx-auto">
            ...
            <p className="text-slate-500 text-sm mb-3">
              ...
              <span className="text-orange-600 font-bold">{spotsLeft} spots</span> left this week.
            </p>
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-slate-200 py-10 pb-28 md:pb-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-teal-500 flex items-center justify-center"><Trash2 size={13} className="text-slate-900" /></div>
            <span className="font-black tracking-tight">Bin<span className="text-teal-600">Pilot</span></span>
            <span className="text-slate-500 text-sm ml-2">Mimico Bin Cleaning</span>
          </div>
          <div className="flex items-center gap-4 text-sm text-slate-500">
            <a href="sms:+15197293673&body=BIN CLEAN" className="flex items-center gap-1.5 hover:text-slate-700 transition-colors"><Phone size={13} />519-729-3673</a>
            <a href="mailto:binpilotmimico@gmail.com" className="flex items-center gap-1.5 hover:text-slate-700 transition-colors"><Mail size={13} />binpilotmimico@gmail.com</a>
          </div>
        </div>
      ...
      {showPopup && (
        <div className="fixed z-50 left-3 right-3 bottom-24 sm:left-6 sm:right-auto sm:bottom-6 sm:max-w-sm fade-in-up">
          <div className="relative bg-white border border-teal-500/30 rounded-2xl shadow-2xl shadow-teal-500/10 p-4 flex items-center gap-3">
            <button onClick={dismissPopup} aria-label="Close" className="absolute top-2 right-2 text-slate-500 hover:text-slate-900 transition-colors">
              <X size={16} />
            </button>
            <img src="/raccoons/peek.png" alt="A raccoon peeking out of a bin" className="w-16 h-16 rounded-xl object-cover bg-white flex-shrink-0" />
            <div className="pr-4">
              <p className="text-sm font-black text-slate-900 leading-snug">
                Only <span className="text-orange-600">{spotsLeft} spots</span> left for Tuesday/Wednesday this week!
              </p>
              <a href="#book" onClick={dismissPopup} className="inline-flex items-center gap-1 mt-1.5 text-xs font-bold text-teal-600 hover:text-teal-300 transition-colors">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#faf7f0]/95 backdrop-blur-md border-t border-slate-200 px-4 py-3 flex gap-3">
        <a href="sms:+15197293673&body=BIN CLEAN" className="flex-1 inline-flex items-center justify-center gap-1.5 bg-slate-100 border border-slate-200 text-slate-900 font-bold text-sm py-3 rounded-xl">
          <Phone size={15} /> Text
        </a>
        <a href="#book" className="flex-[2] inline-flex items-center justify-center gap-1.5 bg-gradient-to-r from-orange-500 to-amber-500 text-slate-900 font-black text-sm py-3 rounded-xl glow-pulse-orange">
          Book My Clean →
        </a>
      </div>
    </div>
